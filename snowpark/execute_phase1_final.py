"""
Phase 1 Execution Script (Final Working Version)
...
def initialize_database(session: Session):
    ...
    try:
        ...
        session.sql("GRANT USAGE ON SCHEMA RAW TO ROLE PUBLIC").collect()
        session.sql("GRANT USAGE ON SCHEMA CORE TO ROLE PUBLIC").collect()
        session.sql("GRANT USAGE ON SCHEMA FEAT TO ROLE PUBLIC").collect()
        session.sql("GRANT USAGE ON SCHEMA ML TO ROLE PUBLIC").collect()
        session.sql("GRANT USAGE ON SCHEMA OPS TO ROLE PUBLIC").collect()
        ...
        logger.info("Inserting audit record...")
        session.sql("""
